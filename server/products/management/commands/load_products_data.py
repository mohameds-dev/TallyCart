...
class Command(BaseCommand):
    ...
    and price snapshots. It uses serializers for robust validation and
    data cleaning before committing to the database.

    The process is separated into two main phases for each row:
    1.  **Validation and Cleaning**: Each row is validated using CSVRowSerializer
        for essential data and cleaned (e.g., parsing dates and decimals).
    ...
    def _filter_and_clean_data(self, reader):
        """
        Reads the CSV, validates and cleans each row using CSVRowSerializer,
        and returns a list of clean data.
        """
        cleaned_data = []
        serializer = CSVRowSerializer()
        
        for row_num, row in enumerate(reader, start=2):
            try:
                # Use serializer to validate and clean the row
                if serializer.is_valid(data=row):
                    cleaned_data.append(serializer.validated_data)
                else:
                    # Handle validation errors
                    self._handle_validation_errors(serializer.errors, row, row_num)
                    self.stats['skipped_invalid'] += 1
                    
            except Exception as e:
                self.stats['skipped_invalid'] += 1
                self.stdout.write(self.style.ERROR(f"Skipping row {row_num} due to unexpected error: {e}"))
                
        return cleaned_data

    def _handle_validation_errors(self, errors, row, row_num):
        """Handle and log validation errors from the serializer."""
        error_messages = []
        ...
        self.stdout.write(self.style.WARNING(
            f"Skipping row {row_num}: {error_summary}\nRow content: {str(row)[:70]}..."
        ))
    ...
    def _get_or_create_shop(self, data):
        ...
        try:
            ...
            shop = self._create_object(
                Shop, 
                name=store_name, 
                address=data.get('store_location', '').strip() or None
            )
    ...
    def _get_or_create_product(self, data):
        ...
        try:
            ...
            product = self._create_object(
                Product,
                name=product_name,
                store_product_id=data.get('store_product_id', '').strip() or None
            )
